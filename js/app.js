(() => {
    ...
    const formElement = document.querySelector(".form-section__inner");
    const popupElement = document.querySelector(".popup");
    const buttonPopup = document.querySelector(".popup__button");
    const bgOverlay = document.querySelector(".bg-overlay");
    const inputsFormElement = document.querySelectorAll(".content-form-section__element");
    if (formElement) formElement.addEventListener("submit", function(e) {
        e.preventDefault();
        if (popupElement && bgOverlay) {
            popupElement.classList.toggle("_active");
            bgOverlay.classList.toggle("_active");
            if (popupElement.classList.contains("_active")) htmlElement.classList.toggle("lock");
        }
        inputsFormElement.forEach(function(inputFormElement) {
            inputFormElement.value = "";
        });
    });
    if (popupElement) if (buttonPopup && bgOverlay) {
        buttonPopup.addEventListener("click", function() {
            popupElement.classList.remove("_active");
            bgOverlay.classList.remove("_active");
    ...
    window.addEventListener("load", function() {
        if (window.innerWidth <= 600) {
            ...
            if (tabsBody) tabsBody.forEach(function(tabBody) {
